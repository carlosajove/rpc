...
from simulator.pybullet.rl.rl_mpc_freq.envs.only_rl_low_freq_Ly_10 import DracoEnvMpcFreq_ONLY_RL_low_freq_Ly_10

from config.draco.pybullet_simulation import Config

model_dir = cwd + "/rl_model/freq_env/Ly_10_higher_lr_Ly_10_only_128/PPO"
env_dir = cwd + "/rl_env/freq_env/Ly_10_higher_lr_Ly_10_only_128/PPO"
#import tracemalloc
import argparse
import torch
new_model = True
...
if __name__ == "__main__":
    ...
    render = False
    env = DracoEnvMpcFreq_ONLY_RL_low_freq_Ly_10(mpc_freq, sim_dt, reduced_obs_size=reduced_obs_size, render=render)
    ...
    save_dir = str1 + f"ONLY_RL_batch_{batch_size_}_nsteps{n_steps_}_low_freq" 
    load_dir = str1 + f"ONLY_RL_batch_{batch_size_}_nsteps{n_steps_}_low_freq" 
    load_path = os.path.join(model_dir, load_dir) 
    save_path = os.path.join(model_dir, save_dir)      
    ## train model
    #policy_kwargs = { 'full_std': False}
    if new_model:
        tensorboard_dir = cwd + "/rl_log/freq_env/ONLY_RL_low_freq_higher_lr_Ly_10_128/"
        ...
        """Dont start with zeros the only rl
    ...
    while(True):
        try:
            model.learn(total_timesteps=TIMESTEPS, 
                        ...
                        tb_log_name=save_dir)#, callback=eval_callback)
            endTime = time.time()
