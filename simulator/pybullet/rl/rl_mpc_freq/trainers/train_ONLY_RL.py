...
if __name__ == "__main__":
    ...
    save_dir = str1 + f"ONLY_RL_batch_{batch_size_}_nsteps{n_steps_}_plus" 
    load_dir = str1 + f"ONLY_RL_batch_{batch_size_}_nsteps{n_steps_}_plus" 
    load_path = os.path.join(model_dir, load_dir) 
    save_path = os.path.join(model_dir, save_dir)      
    ## train model
    #policy_kwargs = { 'full_std': False}
    if new_model:
        tensorboard_dir = cwd + "/rl_log/freq_env/ONLY_RL/"
