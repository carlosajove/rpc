...
model_dir = cwd + "/rl_model/freq_env/tilted_ground/PPO"
env_dir = cwd + "/rl_env/freq_env/tilted_ground/PPO"
#import tracemalloc
import argparse
import torch
new_model = True
...
if __name__ == "__main__":
    ...
    render = False
    env = DracoEnvMpcFreq_tilted_ground_Ly_15(mpc_freq, sim_dt, reduced_obs_size=reduced_obs_size, render = True, disturbance=True)

    monitor_env = Monitor(env)
    vec_env = DummyVecEnv([lambda: monitor_env])

    #MODEL EVALUATION
    eval_env = DracoEnvMpcFreq_tilted_ground_Ly_15(mpc_freq, sim_dt,reduced_obs_size=reduced_obs_size, render = render, disturbance=True)
