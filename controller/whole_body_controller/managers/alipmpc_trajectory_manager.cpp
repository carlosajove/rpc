...
void AlipMpcTrajectoryManager::MpcSolutions(
  ...
  rotationMatrix << std::cos(com_yaw_), std::sin(com_yaw_), -std::sin(com_yaw_),
      std::cos(com_yaw_);
...
void AlipMpcTrajectoryManager::OutputMpcToInertiaCoordinates(const double &kx,
                                                             const double &ky) {
  // COM
  ...
  double sw_end_z =
      this->ComputeZpos(sw_end_x, sw_end_y, 0, kx,
                        ky); // assumes flat surface
  ...
  Swingfoot_end(2) =
      this->ComputeZpos(Swingfoot_end(0), Swingfoot_end(1), 0, kx, ky);
}
...
void AlipMpcTrajectoryManager::GenerateTrajs(
  ...
  double mid_height = (Swingfoot_start(2) + Swingfoot_end(2)) / 2.0;
  AlipSwingPos2->Initialize(Swingfoot_start, Swingfoot_end,
  ...
  // AlipSwingPos2 =
  // new AlipSwing2(Swingfoot_start, Swingfoot_end, traj_height, indata.Ts);

  // swfoot_ori_curve_ = new HermiteQuaternionCurve(
  // start_swfoot_quat_, Eigen::Vector3d::Zero(), des_end_swfoot_quat_,
  // Eigen::Vector3d::Zero(), indata.Ts);

  // if (ori_traj) {
  // torso_ori_curve_ = new HermiteQuaternionCurve(
  // start_torso_quat_, Eigen::Vector3d::Zero(), des_end_torso_quat_,
  // Eigen::Vector3d::Zero(), indata.Ts);
  //}
}
...
void AlipMpcTrajectoryManager::UpdateDesired(const double t) {
  ...
  if (indata.stance_leg == 1) { // update left
    ...
    // double curr_com_x = com_xy_task->CurrentPos()(0);
    // double curr_com_y = com_xy_task->CurrentPos()(1);
    // double z_pos_tilted_plane =
    // ComputeZpos(curr_com_x, curr_com_y, 0, indata.kx, indata.ky);
    // com_z_des = z_pos_tilted_plane + refzH;
