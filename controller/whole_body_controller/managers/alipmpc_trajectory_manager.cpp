...
AlipMpcTrajectoryManager::AlipMpcTrajectoryManager(
  ...
  */
  ...
  cb_tm_ = new CubicBeizerTrajectoryManager<double>();
...
void AlipMpcTrajectoryManager::initializeOri(const double kx) {
  ...
  AdaptOriToTerrain(des_end_torso_quat_, kx);
...
void AlipMpcTrajectoryManager::AdaptOriToTerrain(Eigen::Isometry3d &iso,
                                                 const double kx) {
  iso.linear() = util::SO3FromRPY(0.0, -kx, 0.0) * iso.linear();
  Eigen::Vector3d rpy = util::RPYFromSO3(iso.linear());
  std::cout << "=============================" << std::endl;
  std::cout << "des torso foot ori euler rpy: " << rpy.transpose() << std::endl;
}
void AlipMpcTrajectoryManager::AdaptOriToTerrain(Eigen::Quaterniond &quat,
                                                 const double kx) {
  quat = util::EulerZYXtoQuat(0.0, -kx, 0.0) * quat;
}
void AlipMpcTrajectoryManager::AdaptFootOriToTerrain(const double kx) {
  des_end_swfoot_quat_ =
      util::EulerZYXtoQuat(0.0, -kx, 0.0) * des_end_swfoot_quat_;
  Eigen::Vector3d ypr = util::QuatToEulerZYX(des_end_swfoot_quat_);
  // std::cout << "des swing foot ori euler ypr: " << ypr.transpose() <<
  // std::endl;
}
...
void AlipMpcTrajectoryManager::MpcSolutions(
  ...
  // TODO: add footstep filtering
  // add foot orientation correction if terrain slope is not 0
  // if (kx_ != 0)
  // this->AdaptFootOriToTerrain(kx_);
}
...
void AlipMpcTrajectoryManager::OutputMpcToInertiaCoordinates() {
  ...
  // std::cout << "===================" << std::endl;
  // std::cout << "swing foot z pos: " << Swingfoot_end(2) << std::endl;
}
...
void AlipMpcTrajectoryManager::GenerateTrajs(
  ...
  double mid_height = (Swingfoot_start(2) + Swingfoot_end(2)) / 2;
  AlipSwingPos2 = new AlipSwing2(Swingfoot_start, Swingfoot_end,
                                 mid_height + swing_height, indata.Ts);
...
void AlipMpcTrajectoryManager::GenerateTrajs2(const double &tr_,
                                              const bool &ori_traj) {
  indata.Tr = tr_;
  Eigen::Isometry3d curr_swfoot_iso;
  if (ori_traj)
  ...
  // Alip 2
  // AlipSwingPos2 =
  // new AlipSwing2(Swingfoot_start, Swingfoot_end, swing_height, indata.Ts);

  // cubic beizer curve for foot pos trajectory
  cb_tm_->SetInitialPosition(Swingfoot_start);
  cb_tm_->SetFinalPosition(Swingfoot_end);
  cb_tm_->SetHeight(swing_height);
...
void AlipMpcTrajectoryManager::UpdateDesired2(const double t) {
  ...
  double swing_phase = t / indata.Ts;
  cb_tm_->ComputeSwingTrajectoryBezier(swing_phase, indata.Ts);
  des_swfoot_pos = cb_tm_->GetPosition();
  des_swfoot_vel = cb_tm_->GetVelocity();
  // des_swfoot_acc = cb_tm_->GetAcceleration();
  des_swfoot_acc = Eigen::VectorXd::Zero(3);
  ...
  // desired acceleration & velocity clipping
  // for (int i = 0; i < 3; i++) {
