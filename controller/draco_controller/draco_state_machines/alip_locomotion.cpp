...
AlipLocomotion::AlipLocomotion(StateId state_id, PinocchioRobotSystem *robot,
    ...
    : StateMachine(state_id, robot), ctrl_arch_(ctrl_arch), swing_height_(0.), stance_leg(1){
...
void AlipLocomotion::FirstVisit(){
  ...
  if (stance_leg == 1) {
    ...
    ctrl_arch_->tci_container_->contact_map_["lf_contact"]->SetMaxFz(0.01);
  }
  else {
    ctrl_arch_->tci_container_->contact_map_["lf_contact"]->SetMaxFz(500);
    ctrl_arch_->tci_container_->contact_map_["rf_contact"]->SetMaxFz(0.01);
...
bool AlipLocomotion::SwitchLeg(){  //ahora asume que tocamos en Tr o antes. Que pasa si se atrasa?
  ...
  if (sp_->current_time_ - state_machine_start_time_ > 0.5*Ts){
    ...
    if((stance_leg == 1) && (robot_->GetLinkIsometry(draco_link::l_foot_contact).translation()(2) < 0.02)){
      std::cout << "Right stance to left" << std::endl;
      ...
      state_machine_start_time_ = sp_->current_time_;
      ctrl_arch_->tci_container_->contact_map_["lf_contact"]->SetMaxFz(500);
      ctrl_arch_->tci_container_->contact_map_["rf_contact"]->SetMaxFz(0.001);

      //ctrl_arch_->tci_container_->task_map_["rf_pos"]->SetMaxFz(0.01);


    }    //else if((stance_leg == -1) && (sp_->b_rf_contact_)){
    else if((stance_leg == -1) && (robot_->GetLinkIsometry(draco_link::r_foot_contact).translation()(2) < 0.02)){

      std::cout << "Left stance to right" << std::endl;
      ...
      state_machine_start_time_ = sp_->current_time_;
      ctrl_arch_->tci_container_->contact_map_["rf_contact"]->SetMaxFz(500);
      ctrl_arch_->tci_container_->contact_map_["lf_contact"]->SetMaxFz(0.001);
