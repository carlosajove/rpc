...
// One step(): will be the main loop. While not interrupt do (at the moment no
// interrupt,will walk indifinitely) Get command will call one step one time but
// will perform multiple steps. controller_->GetCommand will be performed inside
// state_machine
AlipLocomotion::AlipLocomotion(StateId state_id, PinocchioRobotSystem *robot,
  ...
  Tr_min_ = 2 * 0.00175; // MPC_FREQ/2 * controller_dt;
}
...
void AlipLocomotion::FirstVisit() { // represents when MPC computation
  ...
  // check if Tr is lower than 0 -> if Tr = Ts;
  if (Tr < 0)
    Tr = Tr_min_;

  // solve mpc & get solutions
  ctrl_arch_->alip_tm_->MpcSolutions(Tr, stance_leg, sp_->Lx_offset_des_,
                                     sp_->Ly_des_, sp_->des_com_yaw_, sp_->kx_,
                                     sp_->ky_, sp_->mu_, new_leg);

  if (sp_->des_com_yaw_ != 0)
    ctrl_arch_->alip_tm_->turning_self_collision();

  sp_->full_policy_ =
      ctrl_arch_->alip_tm_->add_residual_rl_action(sp_->res_rl_action_);

  ctrl_arch_->alip_tm_->GenerateTrajs(Tr, new_leg);
  // ctrl_arch_->alip_tm_->GenerateTrajs2(Tr, new_leg);
  new_leg = false;
  ...
  if (stance_leg == 1) {
    ...
    ctrl_arch_->tci_container_->contact_map_["rf_contact"]->SetMaxFz(
        rf_z_MAX_); // rf_z_MAX_);
    ctrl_arch_->tci_container_->contact_map_["lf_contact"]->SetMaxFz(
        rf_z_max_); // rf_z_max_);
  } else {
    ctrl_arch_->tci_container_->contact_map_["lf_contact"]->SetMaxFz(
        rf_z_MAX_); // rf_z_MAX_);
...
void AlipLocomotion::OneStep() {
  ...
  // ctrl_arch_->alip_tm_->UpdateDesired2(t);
}
...
bool AlipLocomotion::SwitchLeg() { // ahora asume que tocamos en Tr o antes. Que
                                   // pasa si se atrasa?
  ...
  if (sp_->current_time_ - state_machine_start_time_ > 0.5 * Ts) {
    // if (sp_->current_time_ - state_machine_start_time_ > Ts) {
    // std::cout << "[Normal Contact] detected!" << std::endl;
    // if (stance_leg == 1) {
    // stance_leg *= -1;
    ...
    // state_machine_start_time_ =
    // sp_->current_time_; // this should go to new leg
    ...
    // state_machine_start_time_ =
    // sp_->current_time_; // same thing than before
    ...
    //}
    // if ((stance_leg == 1) && (sp_->b_lf_contact_)){  //right stance, left
    // swing
    // TODO: make either projected threshold or force-based contact detection
    // if ((stance_leg == 1) &&
    //(robot_->GetLinkIsometry(draco_link::l_foot_contact).translation()(2) <
    // 0.00005)) {
    if ((stance_leg == 1) && sp_->b_lf_contact_) {
      // if (verbose){
      // util::PrettyConstructor(2, "Switch Leg AlipLocomotion true ");
      // std::cout << "Right stance to left" << " | Tr:" << Tr << "  | state
      // machine time:" << state_machine_time_  <<std::endl;
      //}
      stance_leg *= -1;
      // ctrl_arch_->alip_tm_->RToLstance();
      // update the force managers
    ...
    // else if ((stance_leg == -1) &&
    //(robot_->GetLinkIsometry(draco_link::r_foot_contact)
    //.translation()(2) < 0.00005)) {
    else if ((stance_leg == -1) && sp_->b_rf_contact_) {
      // if (verbose){
      // util::PrettyConstructor(2, "Switch Leg AlipLocomotion true ");
      // std::cout << "Left stance to right" << " | Tr:" << Tr << "  | state
      // machine time:" << state_machine_time_ <<std::endl;
      //}
      stance_leg *= -1;
