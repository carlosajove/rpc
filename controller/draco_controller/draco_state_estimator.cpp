...
void DracoStateEstimator::Update(DracoSensorData *sensor_data) {
  ...
  sp_->b_lf_contact_ = (sensor_data->b_lf_contact_);
  sp_->b_rf_contact_ = (sensor_data->b_rf_contact_);
...
void DracoStateEstimator::UpdateWbcObs() {
  ...
  Eigen::Vector3d torso_com_ang_vel =
      robot_->GetLinkSpatialVel(draco_link::torso_com_link).head<3>();
  ...
  Eigen::Vector3d com_pos_stfoot_torso_ori =
      des_torso_iso.linear().transpose() * com_pos_stfoot;
  Eigen::Vector3d com_vel_torso_ori =
      des_torso_iso.linear().transpose() * com_vel;
  ...
  sp_->torso_roll_pitch_yaw_ =
      util::QuatToEulerZYX(Eigen::Quaterniond(torso_iso.linear()));
  sp_->swfoot_roll_pitch_yaw_ =
      util::QuatToEulerZYX(Eigen::Quaterniond(swfoot_iso.linear()));
  ...
  torso_iso_des_frame.linear() =
      des_torso_iso.linear().transpose() * torso_iso_des_frame.linear();
  swfoot_iso_des_frame.linear() =
      des_torso_iso.linear().transpose() * swfoot_iso_des_frame.linear();

  sp_->torso_rpy_des_frame =
      util::QuatToEulerZYX(Eigen::Quaterniond(torso_iso_des_frame.linear()));
  sp_->swfoot_rpy_des_frame =
      util::QuatToEulerZYX(Eigen::Quaterniond(swfoot_iso_des_frame.linear()));
}
