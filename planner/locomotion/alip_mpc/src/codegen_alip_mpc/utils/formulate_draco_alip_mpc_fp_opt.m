function [] = formulate_draco_alip_mpc_fp_opt(mpc_info)
import casadi.*
...
opti_LS.subject_to(p_ufp_min(1) <= Ufp_traj(1,1)-p_x_init(1) <= p_ufp_max(1))
opti_RS.subject_to(p_ufp_min(1) <= Ufp_traj(1,1)-p_x_init(1) <= p_ufp_max(1))
...
% Left Stance
opti_LS.subject_to(-ufp_y_max <= Ufp_traj(2,1)-p_x_init(2) <= -ufp_y_min);

for n = 2:2:N_steps_ahead-1
    % y foot placement constraint (don't allow crossover)
    opti_LS.subject_to(-ufp_y_max <= Ufp_traj(2,n+1)-X_traj(2,n) <= -ufp_y_min);
    opti_LS.subject_to(ufp_y_min <= Ufp_traj(2,n)-X_traj(2,n-1) <= ufp_y_max);
end

% Right Stance
opti_RS.subject_to(ufp_y_min <= Ufp_traj(2,1)-p_x_init(2) <= ufp_y_max);
for n = 2:2:N_steps_ahead-1
    opti_RS.subject_to(ufp_y_min <= Ufp_traj(2,n+1)-X_traj(2,n) <= ufp_y_max);
    opti_RS.subject_to(-ufp_y_max <= Ufp_traj(2,n)-X_traj(2,n-1) <= -ufp_y_min);
end
...
if sol_type == "qrqp"
    ...
    if compile_src
        ...
        tic
        cg.generate()
        movefile([name_cg_LS '.c'],['gen/draco_mpc_fp_solvers/' name_cg_LS '.c'])
        ...
        tic
        cg.generate()
        movefile([name_cg_RS '.c'],['gen/draco_mpc_fp_solvers/' name_cg_RS '.c'])
        disp("Generation time = " + toc);
    end
