...
Lx_plus = 0.5 * MpcMassCom * MpczH * MpcLegWidth * l * np.tanh(
    0.5 * MpcTs * l) + MpcLx_off
Lx_minus = -0.5 * MpcMassCom * MpczH * MpcLegWidth * l * np.tanh(
    0.5 * MpcTs * l) + MpcLx_off
...
plt.ylabel(r'$\frac{L_{x}}{mz_{H}} (ms^{-1})$',
           fontsize=20,
           rotation='horizontal',
           labelpad=20)
plt.legend(fontsize=16)
...
color = 'tab:blue'
ax1.set_xlabel('Time (s)', fontsize=20)
ax1.set_ylabel(r'$\frac{L_{x}}{mz_{H}}$',
               fontsize=20,
               rotation='horizontal',
               color='black')
