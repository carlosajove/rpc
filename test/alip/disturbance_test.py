...
def disturbance_diagram(mpc_data, rl_data):
    ...
    plt.plot(mpc_data[:, 1] / 39.15,
             mpc_data[:, 0] / 39.15,
             linestyle=':',
             color='tab:blue')  #, color='#FFA400')
    plt.plot(mpc_data[:, 3] / 39.15,
             mpc_data[:, 2] / 39.15,
             linestyle=':',
             color='tab:orange')  #color='#E22A32')
    plt.plot(rl_data[:, 1] / 39.15, rl_data[:, 0] / 39.15,
             color='tab:blue')  #, color='#FFA400')
    ...
    y_axis = max(max(mpc_data[:, 0] / 39.15), max(rl_data[:, 0] / 39.15),
                 max(mpc_data[:, 2] / 39.15),
                 max(rl_data[:, 2] / 39.15))  # Adjust as needed
    x_axis = max(max(mpc_data[:, 1] / 39.15), max(mpc_data[:, 3] / 39.15),
                 max(rl_data[:, 1] / 39.15), max(rl_data[:, 3] / 39.15))
    plt.annotate('walking direction',
                 xy=(0.21, 0.8),
                 xytext=(0.135, 0.7),
...
def disturbance_diagram_2(mpc_data, rl_data):
    ...
    # Plot data
    ax.plot(mpc_data[:, 1] / 39.15,
            mpc_data[:, 0] / 39.15,
            linestyle='dashdot',
            label='MPC')
    ax.plot(mpc_data[:, 3] / 39.15,
            mpc_data[:, 2] / 39.15,
            linestyle='dashdot',
            color='#777777',
            label='MPC+RL')
    ...
    ax.imshow(logo, extent=[-10, 10, -10, 10], aspect='auto',
              zorder=10)  # Set zorder to make sure it's on top
...
if __name__ == "__main__":
    ...
    MID_max_MPC = np.stack(
        (np.array([70, 0, 120, 0]), np.array([0, 100, 0, 160]),
         np.array([-130, 0, -160, 0]), np.array(
             [0, -100, 0, -180]), np.array([70, 0, 120, 0])),
        axis=0)

    MID_max_RL = np.stack(
        (np.array([160, 0, 240, 0]), np.array([0, 120, 0, 170]),
         np.array([-160, 0, -240, 0]), np.array(
             [0, -120, 0, -200]), np.array([160, 0, 240, 0])),
        axis=0)

    disturbance_diagram(MID_max_MPC, MID_max_RL)

    LONG_max_MPC = np.stack((np.array([10, 0, 25, 0]), np.array(
        [0, 15, 0, 25]), np.array([-25, 0, -45, 0]), np.array(
            [0, -15, 0, -20]), np.array([10, 0, 25, 0])),
                            axis=0)

    LONG_max_RL = np.stack((np.array([35, 0, 45, 0]), np.array(
        [0, 30, 0, 35]), np.array([-45, 0, -55, 0]), np.array(
            [0, -20, 0, -25]), np.array([35, 0, 45, 0])),
                           axis=0)

    disturbance_diagram(LONG_max_MPC, LONG_max_RL)

    # SHORT_max_MPC = np.stack(
    # (np.array([350, 0, 400, 0]), np.array([0, 800, 0, 950]),
    # np.array([-450, 0, -550, 0]), np.array(
    # [0, -500, 0, -750]), np.array([350, 0, 400, 0])),
    # axis=0)

    # SHORT_max_RL = np.stack(
    # (np.array([850, 0, 1250, 0]), np.array([0, 650, 0, 850]),
    # np.array([-450, 0, -550, 0]), np.array(
    # [0, -800, 0, -1050]), np.array([850, 0, 1250, 0])),
    # axis=0)
    SHORT_max_MPC = np.stack(
        (np.array([350, 0, 400, 0]), np.array([0, 700, 0, 850]),
         np.array([-450, 0, -550, 0]), np.array(
             [0, -500, 0, -750]), np.array([350, 0, 400, 0])),
        axis=0)
    SHORT_max_RL = np.stack(
        (np.array([850, 0, 1250, 0]), np.array([0, 800, 0, 1000]),
         np.array([-450, 0, -550, 0]), np.array(
             [0, -800, 0, -1050]), np.array([850, 0, 1250, 0])),
        axis=0)
    ...
    MID_max_MPC = np.stack(
        (np.array([80, 0, 90, 0]), np.array([0, 20, 0, 30]),
         np.array([-110, 0, -120, 0]), np.array([0, -30, 0, -40
                                                 ]), np.array([80, 0, 90, 0])),
        axis=0)

    MID_max_RL = np.stack(
        (np.array([90, 0, 110, 0]), np.array([0, 60, 0, 70]),
         np.array([-100, 0, -110, 0]), np.array(
             [0, -50, 0, -60]), np.array([90, 0, 110, 0])),
        axis=0)

    disturbance_diagram(MID_max_MPC, MID_max_RL)

    LONG_max_MPC = np.stack((np.array([20, 0, 20, 0]), np.array(
        [0, 5, 0, 5]), np.array([-30, 0, -40, 0]), np.array(
            [0, -5, 0, -5]), np.array([20, 0, 20, 0])),
                            axis=0)

    LONG_max_RL = np.stack((np.array([20, 0, 30, 0]), np.array(
        [0, 15, 0, 20]), np.array([-30, 0, -35, 0]), np.array(
            [0, -15, 0, -20]), np.array([20, 0, 30, 0])),
                           axis=0)

    disturbance_diagram(LONG_max_MPC, LONG_max_RL)

    SHORT_max_MPC = np.stack(
        (np.array([350, 0, 350, 0]), np.array([0, 150, 0, 200]),
         np.array([-650, 0, -700, 0]), np.array(
             [0, -250, 0, -300]), np.array([350, 0, 400, 0])),
        axis=0)

    SHORT_max_RL = np.stack(
        (np.array([350, 0, 400, 0]), np.array([0, 350, 0, 400]),
         np.array([-700, 0, -750, 0]), np.array(
             [0, -400, 0, -450]), np.array([350, 0, 400, 0])),
        axis=0)
