...
def alip_trajectory(x0, dt):  #x = [x y Lx Ly]
    ...
    zH = 0.69
    g = 9.81
    A = np.array([[0, 0, 0, 1 / (mass * zH)], [0, 0, -1 / (mass * zH), 0],
                  [0, -mass * g, 0, 0], [mass * g, 0, 0, 0]])
    Adt = expm(A * dt)
...
alip_state_traj = []
alip_end_of_step_predicted_state_traj = []
alip_time_traj = []
for i in range(np.size(traj_indices) - 2):
    x0 = np.array([
        MpcxCOM[traj_indices[i] + 4], MpcyCOM[traj_indices[i] + 4],
        MpcLxCOM[traj_indices[i] + 4], MpcLyCOM[traj_indices[i] + 4]
    ])
    for j in range(traj_indices[i] + 4, traj_indices[i + 1]):
        dt = time[j] - time[traj_indices[i]]
        Tr = time[traj_indices[i + 1]] - time[j]
        x_measured = np.array(
            [mpc_coor_x[j], mpc_coor_y[j], mpc_coor_L_x[j], mpc_coor_L_y[j]])
        print(x_measured)
        res = alip_trajectory(x0, dt)
        predicted_res = alip_trajectory(x_measured, Tr)
        alip_state_traj.append(res)
        alip_end_of_step_predicted_state_traj.append(predicted_res)
...
def compute_intra_step_traj(start_time, end_time):
    minitraj = []
    minitraj_time = []
    mini_traj_x0 = np.array([
        mpc_coor_x[start_time], mpc_coor_y[start_time],
        mpc_coor_L_x[start_time], mpc_coor_L_y[start_time]
    ])
...
alip_end_of_step_predicted_state_traj = np.stack(
    alip_end_of_step_predicted_state_traj)
...
def plot_alip_traj_vs_real(dim,
                           mpc_coor_,
                           name_robot,
                           name_pred,
                           name_pred_end,
    ...
    # plt.plot(time[traj_indices[0]+5:traj_indices[-2]], mpc_coor_[traj_indices[0]+5:traj_indices[-2]], label="real " + name)
    real_traj_mask = np.abs(
        np.diff(mpc_coor_[traj_indices[0]:traj_indices[-2]],
                prepend=mpc_coor_[traj_indices[0]])) < eps1
    ...
    plt.plot(time[traj_indices[0]:traj_indices[-2]],
             mpc_coor_[traj_indices[0]:traj_indices[-2]],
             label=name_robot)
    # Calculate mask for points with discontinuities greater than epsilon
    mask = np.abs(
        np.diff(alip_state_traj[:, dim], prepend=alip_state_traj[0,
                                                                 dim])) < eps2
    ...
    # if (dim == 3):
    # plt.plot(alip_time_traj,
    # alip_end_of_step_predicted_state_traj[:, dim],
    # label=name_pred_end)
    # else:
    ...
    plt.plot(alip_time_traj,
             ...
             label=name_pred_end)
    # if (dim == 3):
    # plt.plot(alip_time_traj, (1 / (mass * zH)) *
    # alip_end_of_step_predicted_state_traj[:, dim],
    # label=name_pred_end)
    # plt.plot(time[traj_indices[0]:traj_indices[-2]], (1 / (mass * zH)) *
    # mpc_coor_[traj_indices[0]:traj_indices[-2]],
    # label=name_robot)
    for i in range(len(landingTimes_in_range) - 1):
        plt.axvspan(landingTimes_in_range[i],
...
# Plot alip state
plot_alip_traj_vs_real(0,
                       mpc_coor_x,
                       name_robot="$x$",
                       name_pred="$x^{pred}$",
                       name_pred_end="$x^{pred,end}$",
                       axis_y="$x \,(m)$",
                       eps1=1e-3,
                       eps2=1e-3)
plot_alip_traj_vs_real(1,
                       mpc_coor_y,
                       name_robot="$y$",
                       name_pred="$y^{pred}$",
                       name_pred_end="$y^{pred,end}$",
                       axis_y="$y \,(m)$",
                       eps1=1e-2,
                       eps2=1e-2)
plot_alip_traj_vs_real(2,
                       mpc_coor_L_x,
                       name_robot="$L_{x}$",
                       name_pred="$L_{x}^{pred}$",
                       name_pred_end="$L_{x}^{pred,end}$",
                       axis_y="$L_{x} \, (kg \dot m^{2} \dot s^{-1})$",
                       eps1=1e-1,
                       eps2=1e-1)
plot_alip_traj_vs_real(3,
                       mpc_coor_L_y,
                       name_robot="$L_{y}$",
                       name_pred="$L_{y}^{pred}$",
                       name_pred_end="$L_{y}^{pred,end}$",
                       axis_y="$L_{y} \, (kg \dot m^{2} \dot s^{-1})$",
                       eps1=0.075,
                       eps2=0.05)
...
plt.figure(figsize=(9, 6))
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         mpc_coor_L_x[traj_indices[0]:traj_indices[-2]],
         label='$L^{x}$')
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         mpc_coor_Lc_x[traj_indices[0]:traj_indices[-2]],
         label='$L^{x}_{c}$')
for i in range(len(landingTimes_in_range) - 1):
    plt.axvspan(landingTimes_in_range[i],
                landingTimes_in_range[i + 1],
...
plt.figure(figsize=(9, 6))
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         mpc_coor_L_y[traj_indices[0]:traj_indices[-2]],
         label='$L^{y}$')
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         mpc_coor_Lc_y[traj_indices[0]:traj_indices[-2]],
         label='$L^{y}_{c}$')
for i in range(len(landingTimes_in_range) - 1):
    plt.axvspan(landingTimes_in_range[i],
                landingTimes_in_range[i + 1],
...
plt.figure(figsize=(9, 6))
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         mpc_coor_L_z[traj_indices[0]:traj_indices[-2]],
         label='$L^{z}$')
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         mpc_coor_Lc_z[traj_indices[0]:traj_indices[-2]],
         label='$L^{z}_{c}$')
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         mpc_coor_L_z[traj_indices[0]:traj_indices[-2]] -
         mpc_coor_Lc_z[traj_indices[0]:traj_indices[-2]],
...
plt.figure(figsize=(9, 6))
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         mpc_coor_z[traj_indices[0]:traj_indices[-2]] - 0.003545,
         label='$z$')
plt.plot(time[traj_indices[0]:traj_indices[-2]],
         MpczCOM[traj_indices[0]:traj_indices[-2]],
         label='$z_{H}$')
for i in range(len(landingTimes_in_range) - 1):
    plt.axvspan(landingTimes_in_range[i],
                landingTimes_in_range[i + 1],
...
plt.figure(figsize=(9, 6))
plt.plot(time[one_inter],
         mpc_coor_L_y[one_inter],
         color='#1f77b4',
         label='$L_y$')
